...
void send_segment(PNETSIM_SOCKET s)
{
	...
	NetSim_BUFFER* b = DEVICE_SOCKETINTERFACE(s->localDeviceId)->pstruSocketBuffer[s->sId];
	NetSim_PACKET* p = fn_NetSim_Packet_GetPacketFromBuffer(b, 0);
	if (!p)
	...
	while (p)
	{
		...
		{
			NetSim_PACKET* pt = fn_NetSim_Packet_GetPacketFromBuffer(b, 1);
		...
		p = fn_NetSim_Packet_GetPacketFromBuffer(b, 0);
	}
